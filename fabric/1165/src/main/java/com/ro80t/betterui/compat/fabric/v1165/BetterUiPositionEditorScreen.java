...
public final class BetterUiPositionEditorScreen extends Screen {
    ...
    public BetterUiPositionEditorScreen(final Screen parent) {
        super(new LiteralText("BetterUI - Drag to Reposition"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addButton(new ButtonWidget(backX, TOP_Y, BACK_WIDTH, BUTTON_HEIGHT,
                new LiteralText("Back"), button -> this.onClose()));

        this.addButton(new ButtonWidget(resizeX, TOP_Y, RESIZE_WIDTH, BUTTON_HEIGHT, resizeLabel(), button -> {
            this.resizeMode = !this.resizeMode;
            button.setMessage(resizeLabel());
        }));

        this.addButton(new ButtonWidget(detailsX, TOP_Y, DETAILS_WIDTH, BUTTON_HEIGHT, detailsToggleLabel(), button -> {
        ...
        final ButtonWidget orientationButton = new ButtonWidget(menuX, menuY, MENU_WIDTH, BUTTON_HEIGHT,
        ...
        this.addButton(orientationButton);
    ...
    private void addDetailsMenuItem(final int x, final int y, final Text label, final Runnable resetAction,
        ...
        final ButtonWidget button = new ButtonWidget(x, y, MENU_WIDTH, BUTTON_HEIGHT, label, b -> {
            ...
            BetterUiMod.saveConfig();
            if (reopenScreen) {
                this.client.openScreen(new BetterUiPositionEditorScreen(this.parent));
            }
        });
    ...
    private static Text orientationLabel() {
        return new LiteralText("Armor: " + (BetterUiMod.getConfig().isArmorHudVertical() ? "Vertical" : "Horizontal"));
    }

    @Override
    public void render(final MatrixStack matrices, final int mouseX, final int mouseY, final float partialTick) {
        super.render(matrices, mouseX, mouseY, partialTick);

        this.handles.clear();

        this.textRenderer.drawWithShadow(matrices, this.resizeMode
                ? "Drag an element to resize it"
                : "Drag an element to move it", 4, 4, 0xFFFFFFFF);

        drawTextHandle(matrices, "FPS", "88 FPS", BetterUiMod.getConfig().getFpsLayout());
        drawTextHandle(matrices, "Coordinates", "X: 12.3 Y: 64.0 Z: -8.5", BetterUiMod.getConfig().getCoordinatesLayout());
        drawArmorHudPreview(matrices);

        if (this.dragging != null) {
            this.textRenderer.drawWithShadow(matrices, "Dragging: " + this.dragging.label(), 4, this.height - 16, 0xFFFF00);
        }
    }

    private void drawTextHandle(final MatrixStack matrices, final String label, final String text, final HudLayout layout) {
        final TextRenderer textRenderer = this.textRenderer;
        final int x = layout.x(this.width);
        final int y = layout.y(this.height);
        final int textWidth = Math.round(textRenderer.getWidth(text) * layout.getScale());
        final int textHeight = Math.round(textRenderer.fontHeight * layout.getScale());

        fill(matrices, x - HANDLE_PADDING, y - HANDLE_PADDING, x + textWidth + HANDLE_PADDING,
                y + textHeight + HANDLE_PADDING, 0x556699FF);
        textRenderer.drawWithShadow(matrices, label + " (" + String.format("%.2f", layout.getScale()) + "x)",
                x - HANDLE_PADDING, y - HANDLE_PADDING - 10, 0xFFFF00);
    ...
    private void drawArmorHudPreview(final MatrixStack matrices) {
        ...
        final int x = layout.x(this.width);
        final int y = layout.y(this.height);
        final int columnWidth = DurabilityHud.columnWidth(textRenderer);
        final int iconSize = DurabilityHud.ICON_SIZE;
        ...
        for (int row = 0; row < ARMOR_SAMPLE_TEXTS.length; row++) {
            final DurabilityHud.Position pos = DurabilityHud.iconPosition(row, vertical, columnWidth);
