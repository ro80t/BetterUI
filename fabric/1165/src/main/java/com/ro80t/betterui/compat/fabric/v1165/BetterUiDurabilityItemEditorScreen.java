...
public final class BetterUiDurabilityItemEditorScreen extends Screen {
    ...
    public BetterUiDurabilityItemEditorScreen(final Screen parent) {
        super(new LiteralText("BetterUI - Item Durability Number"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addButton(new ButtonWidget(resetX, TOP_Y, RESET_WIDTH, BUTTON_HEIGHT,
                new LiteralText("Reset"), button -> {
                    BetterUiMod.getConfig().resetDurabilityItemLayout();
                    BetterUiMod.saveConfig();
                }));
    }

    private LiteralText resizeLabel() {
    ...
    public void render(final MatrixStack matrices, final int mouseX, final int mouseY, final float partialTick) {
        ...
        this.textRenderer.drawWithShadow(matrices, this.resizeMode
                ...
                : "Drag the number to move it", 4, 4, 0xFFFFFFFF);

        drawPreview(matrices);
    }

    private void drawPreview(final MatrixStack matrices) {
        ...
        fill(matrices, slotX - 1, slotY - 1, slotX + 17, slotY + 17, 0x55FFFFFF);
        itemRenderer.renderGuiItemIcon(new ItemStack(Items.DIAMOND_PICKAXE), slotX, slotY);
        ...
        textRenderer.drawWithShadow(matrices, "Item Number (" + String.format("%.2f", layout.getScale()) + "x)",
                x - 40, y - 24, 0xFFFF00);

        matrices.push();
        matrices.translate(x, y, 200.0F);
        matrices.scale(layout.getScale(), layout.getScale(), 1.0F);
        textRenderer.drawWithShadow(matrices, text, -textWidth / 2F, 0, 0xFFFFFFFF);
        matrices.pop();

        final int handleSize = Math.round(Math.max(textRenderer.fontHeight, textWidth) * layout.getScale())
                + HANDLE_PADDING * 2;
